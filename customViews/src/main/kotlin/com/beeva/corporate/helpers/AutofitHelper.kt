package com.beeva.corporate.helpers
...
class AutofitHelper private constructor(// Attributes
  ...
  fun addOnTextSizeChangeListener(listener: OnTextSizeChangeListener): AutofitHelper {
    if (mListeners == null) {
      mListeners = ArrayList<OnTextSizeChangeListener>()
    }
  ...
  companion object {
     ...
     */
    @JvmOverloads fun create(view: TextView, attrs: AttributeSet? = null,
        defStyle: Int = 0): AutofitHelper {
      ...
      if (attrs != null) {
        ...
        helper.setMinTextSize(TypedValue.COMPLEX_UNIT_PX, minTextSize.toFloat()).setPrecision(
            precision)
      }
    ...
    private fun autofit(view: TextView, paint: TextPaint, minTextSize: Float, maxTextSize: Float,
      ...
      if (maxLines == 1 && paint.measureText(text, 0, text.length) > targetWidth || getLineCount(
          text, paint, size, targetWidth.toFloat(), displayMetrics) > maxLines) {
        size = getAutofitTextSize(text, paint, targetWidth.toFloat(), maxLines, low, high,
            precision,
            displayMetrics)
      }
     ...
     */
    private fun getAutofitTextSize(text: CharSequence, paint: TextPaint, targetWidth: Float,
        maxLines: Int, low: Float, high: Float, precision: Float,
      ...
      if (maxLines != 1) {
        layout = StaticLayout(text, paint, targetWidth.toInt(), Layout.Alignment.ALIGN_NORMAL, 1.0f,
            0.0f, true)
    ...
    private fun getLineCount(text: CharSequence, paint: TextPaint, size: Float, width: Float,
      ...
      val layout = StaticLayout(text, paint, width.toInt(), Layout.Alignment.ALIGN_NORMAL, 1.0f,
          0.0f, true)
